...
$("teamExcelFile").onchange=async e=>{
  ...
  try{
    ...
    rows.slice(1).forEach((row,i)=>{
      ...
      const name =
        String(row[teamNameIndex] ?? "")
          .replace(/\s+/g," ")
          .trim();

      const budgetRaw =
        String(row[budgetIndex] ?? "")
          .replace(/₹/g,"")
          .replace(/,/g,"")
          .trim();

      const pin =
        String(row[pinIndex] ?? "").trim();

      if(!name && !budgetRaw && !pin)
        return;
      ...
      if(!budgetRaw){
        ...
        errors.push(
          `Row ${line}: Starting Budget is invalid.`
        );
    ...
    if(errors.length){
      ...
      return setMsg(
        "teamMsg",
        `<b>❌ Team Import stopped.</b><br><br>` +
        errors
          .map(e=>`• ${esc(e)}`)
          .join("<br>") +
        `<br><br><b>No team data was changed.</b>`,
        "err"
      );
    ...
    setMsg(
      "teamMsg",
      `✅ ${clean.length} team${clean.length===1?"":"s"} imported successfully.`,
      "ok"
    );
...
$("replaceTeamExcel").onclick=()=>{
  ...
  if(!state.teams.length){
    ...
    return setMsg(
      "teamMsg",
      "No current Team Excel data is loaded.",
      "err"
    );
  ...
  const ok =
    confirm(
      ...
      "The current team list will be removed and " +
      "the next Excel you upload will become the " +
      "new team list.\n\n" +
      "Players, Player Excel and auction history " +
      "will NOT be deleted.\n\n" +
      "Continue?"
    );
  ...
  setMsg(
    "teamMsg",
    "🗑️ Current team data removed. Upload your new Team Excel now. Player Excel and auction history are still safe.",
    "ok"
  );
