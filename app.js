...
/* =========================================
   ONE BY ONE AUCTION PLAYER SYSTEM
   ========================================= */
...
(function(){
  ...
  function getAuctionPlayers(){
    ...
    return players.filter(function(player){
      ...
      /* Already sold players remove */
      const alreadySold =
        Array.isArray(state.sold) &&
        state.sold.some(function(sold){
          return String(sold.playerId) === String(player.id);
        });
  ...
  function showNextAuctionPlayer(){
    ...
    if(auctionPlayerIndex >= auctionPlayers.length){

      auctionPlayerIndex = auctionPlayers.length - 1;

    }


    const player =
      auctionPlayers[auctionPlayerIndex];
    ...
    if(playerInfo){
      ...
      playerInfo.innerHTML = `
        <div style="
          ...
          margin-bottom:6px;
        ">
          ${typeof esc === "function" ? esc(player.name || "") : (player.name || "")}
        </div>

        <div>
          Category:
          <b>
            ${typeof esc === "function" ? esc(player.category || "") : (player.category || "")}
          </b>
        </div>
  ...
  setTimeout(function(){
    ...
    showNextAuctionPlayer();

  }, 300);


  /* Make function available globally */
  window.showNextAuctionPlayer =
    showNextAuctionPlayer;


  window.goToNextAuctionPlayer =
    function(){
...
/* =========================================
   SOLD / UNSOLD + AUTO NEXT + UNDO / REDO
   ========================================= */
...
(function(){
  ...
  function getCurrentPlayer(){
    ...
    return window.currentAuctionPlayer || null;

  }


  function saveAuctionSnapshot(){
  ...
  function restoreAuctionSnapshot(snapshot){
    ...
    state.sold = JSON.parse(
      JSON.stringify(snapshot.sold || [])
    );
  ...
  window.handleAuctionSold = function(){
    ...
    if(!player){
      ...
      alert("⚠️ No current player selected.");
    ...
    const teamInput =
      document.getElementById("auctionTeam");

    const bidInput =
      document.getElementById("bidPrice");


    const teamName =
      teamInput ? teamInput.value.trim() : "";

    const bid =
      bidInput ? Number(bidInput.value) : 0;


    if(!teamName){

      alert("⚠️ Please select Team Name.");
    ...
    if(!bid || bid <= 0){
      ...
      alert("⚠️ Please enter Bid Amount.");
    ...
    /* Deduct budget */
    team.remaining =
      Number(team.remaining ?? team.budget ?? 0) - bid;
  ...
  window.undoAuctionAction = function(){
    ...
    restoreAuctionSnapshot(
      previousState
    );


    alert("↩️ Last auction action undone.");
  ...
  window.redoAuctionAction = function(){
    ...
    alert("↪️ Auction action restored.");
